...
言語切り替えボタンの表示/非表示は**設定ドロワー**の「言語切り替え」トグルで制御できます。`settingsStore.showLanguageSwitch` 状態に基づき、`localStorage`（キー: `app-show-language-switch`）に永続化されます。無効にすると、ヘッダーとログインページの言語切り替えボタンが非表示になります。
...
// 言語切り替えボタンの表示を制御
settingsStore.setShowLanguageSwitch(false) // 非表示
settingsStore.setShowLanguageSwitch(true)  // 表示
```
