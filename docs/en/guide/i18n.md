...
The visibility of the language switch button can be controlled via the "Language Switch" toggle in the **Settings Drawer**, backed by `settingsStore.showLanguageSwitch` and persisted to `localStorage` (key: `app-show-language-switch`). When disabled, the language switch button is hidden from both the header and the login page.
...
// Control language switch button visibility
settingsStore.setShowLanguageSwitch(false) // Hide
settingsStore.setShowLanguageSwitch(true)  // Show
```
